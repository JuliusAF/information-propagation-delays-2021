...
# log copying
COPY_INTERVAL = 18000
LOG_PATH = '/home/ubuntu/.litecoin/debug.log'
COPY_PATH = '/home/ubuntu/documents/logs/litecoin/'
COPIED_PATH = '/home/ubuntu/documents/logs/litecoin/debug.log'
SHORTER_PATH = '/home/ubuntu/documents/logs/litecoin/props.log'
TX_PATH = '/home/ubuntu/documents/logs/litecoin/txs.log'
CONN_PATH = '/home/ubuntu/documents/logs/litecoin/conns.log'
...
# copy litecoin log and empty file, then parse copied log
...
def clean_log(src=LOG_PATH, dst=COPY_PATH):
    while True:
        ...
        os.system('litecoin-cli logging "[]" "[\\"net\\"]"')
        time.sleep(.1)
...
PEERS_PATH = '/home/ubuntu/documents/logs/litecoin/peers/'
PEERS_INTERVAL = 7200
# token for ipinfo.io
TOKEN = 'XXX'
...
def get_peer_info():
    result = subprocess.run(['litecoin-cli', 'getpeerinfo'], stdout=subprocess.PIPE)
...
CHAINTIP_PATH = '/home/ubuntu/documents/logs/litecoin/chains/'
CHAINTIP_INTERVAL = 14400


def save_chain_tips():
    result = subprocess.run(['litecoin-cli', 'getchaintips'], stdout=subprocess.PIPE)
...
def get_block_info():
    ...
    try:
        for row in cur.execute("select hash from propagation where timestamp>? group by hash", (latest,)):
            result = subprocess.run(['litecoin-cli', 'getblock', '{}'.format(row[0]), "1"], stdout=subprocess.PIPE)
            ...
            if result and 'error' not in result:
                try:
                    ...
                    continue
                result = subprocess.run(['litecoin-cli', 'getblockstats', '{}'.format(height)], stdout=subprocess.PIPE)
                result = result.stdout.decode("utf-8")
                try:
                    if result and 'error' not in result:
                        cur2 = conn.cursor()
