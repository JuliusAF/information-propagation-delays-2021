...
# log copying
COPY_INTERVAL = 18000
LOG_PATH = '/home/ubuntu/.dogecoin/debug.log'
COPY_PATH = '/home/ubuntu/documents/logs/dogecoin/'
COPIED_PATH = '/home/ubuntu/documents/logs/dogecoin/debug.log'
SHORTER_PATH = '/home/ubuntu/documents/logs/dogecoin/props.log'
TX_PATH = '/home/ubuntu/documents/logs/dogecoin/txs.log'
CONN_PATH = '/home/ubuntu/documents/logs/dogecoin/conns.log'
...
# copy dogecoin log and empty file, then parse copied log
...
def clean_log(src=LOG_PATH, dst=COPY_PATH):
    while True:
        ...
        os.system('dogecoin-cli logging "[]" "[\\"net\\"]"')
        time.sleep(.1)
...
PEERS_PATH = '/home/ubuntu/documents/logs/dogecoin/peers/'
PEERS_INTERVAL = 7200
# token for ipinfo.io
TOKEN = 'XXX'
...
def get_peer_info():
    result = subprocess.run(['dogecoin-cli', 'getpeerinfo'], stdout=subprocess.PIPE)
...
CHAINTIP_PATH = '/home/ubuntu/documents/logs/dogecoin/chains/'
CHAINTIP_INTERVAL = 14400


def save_chain_tips():
    result = subprocess.run(['dogecoin-cli', 'getchaintips'], stdout=subprocess.PIPE)
...
def get_block_info():
    ...
    try:
        for row in cur.execute("select hash from propagation where timestamp>? group by hash", (latest,)):
            result = subprocess.run(['dogecoin-cli', 'getblock', '{}'.format(row[0])], stdout=subprocess.PIPE)
            ...
            if result and 'error' not in result:
                try:
                    ...
                    continue
                result = subprocess.run(['dogecoin-cli', 'getblockstats', '{}'.format(height)], stdout=subprocess.PIPE)
                result = result.stdout.decode("utf-8")
                try:
                    if result and 'error' not in result:
                        cur2 = conn.cursor()
